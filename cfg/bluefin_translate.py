...
BLUEFIN_CURATED = os.path.join(FOLDER, "bluefin.yaml")
BLUEFIN_ORIGINAL = os.path.join(FOLDER, "bluefin_original.yaml")
...
def main() -> None:
    ...
    with open(BLUEFIN_CURATED, "r") as file:
        cfg = yaml.load(file)

    with open(BLUEFIN_ORIGINAL, "r") as file:
        cfg_orig = yaml.load(file)

    for route in args.routes:
        translate_route_inplace(cfg, cfg_orig, route)

    with open(BLUEFIN_CURATED, "w") as file:
        yaml.dump(cfg, file)
